...
mode.addEventListener("change", () => {
    ...
    document.querySelectorAll(".phonemeOutBackend").forEach(el => {
        el.style.display = isWordMode ? "none" : "inline-block";
    });
});
...
document.getElementById("phonemeConvert").onclick = () => {
    ...
    out.textContent =
        mode.value === "word"
            ? wordToBackend(text, ob, { overrides })
            : convertPhonemes(text, { lexicon: overrides });
};

document.getElementById("phonemeConvertAll").onclick = () => {
    const text = inp.value.trim();
    if (!text) return (out.textContent = "");
    ...
    if (mode.value === "word") {
        out.textContent =
            `VTML:
${wordToBackend(text, Backend.VTML, { overrides })}

Balabolka:
${wordToBackend(text, Backend.BAL, { overrides })}

DECtalk:
${wordToBackend(text, Backend.DT, { overrides })}`;
    }

    else {
        out.textContent = `${convertPhonemes(text, { lexicon: overrides })}`;
    }
};
